...
**Objective**: Verify that an API key issued for tenant-a is rejected when validated in the context of tenant-b.
...
**Test Steps**:
1. Send `POST /internal/v1/api-keys/validate` with the tenant-a API key but in the context of tenant-b (e.g., with tenant-b gateway or tenant-b context header)

...
**Objective**: Verify that the subscriptions/select endpoint rejects a request where the tenant context does not match the authenticated identity.
...
**Test Steps**:
1. Send `POST /internal/v1/subscriptions/select` authenticated as tenant-a but with `X-MaaS-Tenant: tenant-b`
